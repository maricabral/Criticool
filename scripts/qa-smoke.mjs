...
async function run() {
  ...
  const userARes = await request('POST', '/auth/register', {
    body: {
      email: `alice_${suffix}@test.local`,
      password: 'Password1!',
  ...
  const userCRes = await request('POST', '/auth/register', {
    body: {
      email: `carol_${suffix}@test.local`,
      password: 'Password3!',
  ...
  assert(
    loginRes.status === 201 || loginRes.status === 200,
    `Login succeeds (status ${loginRes.status})`,
  );
  ...
  assert(
    refreshRes.status === 201 || refreshRes.status === 200,
    `Refresh succeeds (status ${refreshRes.status})`,
  );
  assert(
    refreshRes.data.tokens?.refreshToken !== userA.tokens.refreshToken,
  ...
  // ─── Movie Search ───
  console.log('\n--- Movies: Search ---');
  const searchRes = await request('GET', '/movies/search?q=Inception', {
  ...
  const importRes = await request('POST', `/movies/tmdb/${tmdbId}/import`, {
    token: userA.tokens.accessToken,
  });
  assert(
    importRes.status === 201 || importRes.status === 200,
    `Movie import succeeds (status ${importRes.status})`,
  );
  ...
  const reviewRes = await request('POST', '/reviews', {
    ...
    body: {
      movieId,
      rating: 4.5,
      quickTake: 'Amazing film!',
      containsSpoilers: false,
      tags: ['mind-bending'],
    },
  });
  assert(
    reviewRes.status === 201 || reviewRes.status === 200,
    `Review created (status ${reviewRes.status})`,
  );
  const reviewId = reviewRes.data?.id;
  assert(!!reviewId, 'Review has an id');

  console.log('\n--- Movies: Detail ---');
  const movieDetailARes = await request('GET', `/movies/${movieId}`, {
    token: userA.tokens.accessToken,
  });
  assert(
    movieDetailARes.status === 200,
    `Movie detail loads for reviewer (status ${movieDetailARes.status})`,
  );
  assert(
    movieDetailARes.data?.viewerReview?.id === reviewId,
    'Movie detail includes viewer existing review',
  );
  ...
  assert(
    frReqRes.status === 201 || frReqRes.status === 200,
    `Friend request A→B created (status ${frReqRes.status})`,
  );

  const notifBRequestRes = await request('GET', '/notifications', {
    token: userB.tokens.accessToken,
  });
  assert(notifBRequestRes.status === 200, 'B can load notifications');
  assert(
    notifBRequestRes.data.items?.some?.(
      (item) => item.type === 'friend_request_received' && item.actor?.id === userA.id,
    ),
    'B receives friend request notification',
  );

  const incomingRes = await request('GET', '/friend-requests/incoming', {
    token: userB.tokens.accessToken,
  });
  ...
  const acceptRes = await request('POST', `/friend-requests/${pendingReq?.id}/accept`, {
    token: userB.tokens.accessToken,
  });
  assert(
    acceptRes.status === 201 || acceptRes.status === 200,
    `Friend request accepted (status ${acceptRes.status})`,
  );

  const notifARequestRes = await request('GET', '/notifications', {
    token: userA.tokens.accessToken,
  });
  assert(
    notifARequestRes.data.items?.some?.(
      (item) => item.type === 'friend_request_accepted' && item.actor?.id === userB.id,
    ),
  ...
  const movieDetailBRes = await request('GET', `/movies/${movieId}`, {
    token: userB.tokens.accessToken,
  });
  ...
  const friendReviewsRes = await request('GET', `/feed/users/${userA.id}`, {
    token: userB.tokens.accessToken,
  });
  assert(
    friendReviewsRes.status === 200,
    `Friend profile reviews load (status ${friendReviewsRes.status})`,
  );
  assert(
  ...
  const strangerProfileReviewsRes = await request('GET', `/feed/users/${userA.id}`, {
    token: userC.tokens.accessToken,
  });
  assert(
    strangerProfileReviewsRes.status === 404,
    `Stranger cannot load friend profile reviews (status ${strangerProfileReviewsRes.status})`,
  );

  // ─── Review Detail Visibility ───
  console.log('\n--- Review Detail: Visibility ---');
  const detailBRes = await request('GET', `/reviews/${reviewId}`, {
    token: userB.tokens.accessToken,
  });
  assert(detailBRes.status === 200, 'Friend can access review detail');

  const detailCRes = await request('GET', `/reviews/${reviewId}`, {
    token: userC.tokens.accessToken,
  });
  assert(
    detailCRes.status === 404,
    `Stranger gets 404 for review detail (status ${detailCRes.status})`,
  );
  ...
  assert(
    editReviewRes.data?.quickTake === 'Amazing film, edited!',
    'Edited review returns new quick take',
  );
  ...
  assert(
    translateReviewBRes.status === 201 || translateReviewBRes.status === 200,
    `Friend can translate review (status ${translateReviewBRes.status})`,
  );
  ...
  assert(
    translateReviewCRes.status === 404,
    `Stranger cannot translate review (status ${translateReviewCRes.status})`,
  );
  ...
  assert(
    commentRes.status === 201 || commentRes.status === 200,
    `Comment created (status ${commentRes.status})`,
  );
  const commentId = commentRes.data?.id;

  const notifACommentRes = await request('GET', '/notifications', {
    token: userA.tokens.accessToken,
  });
  assert(
    notifACommentRes.data.items?.some?.(
      (item) => item.type === 'review_commented' && item.commentId === commentId,
    ),
  ...
  assert(
    translateCommentARes.status === 201 || translateCommentARes.status === 200,
    `Review author can translate comment (status ${translateCommentARes.status})`,
  );
  assert(
    translateCommentARes.data?.fields?.body,
    'Comment translation returns user-generated body',
  );
  ...
  assert(
    translateCommentCRes.status === 404,
    `Stranger cannot translate comment (status ${translateCommentCRes.status})`,
  );
  ...
  assert(
    replyRes.status === 201 || replyRes.status === 200,
    `Reply created (status ${replyRes.status})`,
  );
  assert(replyRes.data?.depth === 1, 'Reply has depth 1');
  const replyId = replyRes.data?.id;

  const notifBReplyRes = await request('GET', '/notifications', {
    token: userB.tokens.accessToken,
  });
  assert(
    notifBReplyRes.data.items?.some?.(
      (item) => item.type === 'comment_replied' && item.commentId === replyId,
    ),
  ...
  assert(
    voteRes.status === 201 || voteRes.status === 200,
    `Vote created (status ${voteRes.status})`,
  );
  ...
  assert(
    notifBVoteRes.data.items?.some?.(
      (item) => item.type === 'comment_voted' && item.commentId === commentId,
    ),
    'Comment author receives vote notification',
  );

  const removeVoteRes = await request(
    'DELETE',
    `/reviews/${reviewId}/comments/${commentId}/votes`,
    {
      token: userA.tokens.accessToken,
    },
  );
  ...
  assert(
    deleteCommentRes.status === 200,
    `Comment soft-deleted (status ${deleteCommentRes.status})`,
  );

  const detailAfterCommentDelete = await request('GET', `/reviews/${reviewId}?commentSort=new`, {
    token: userA.tokens.accessToken,
  });
  assert(detailAfterCommentDelete.status === 200, 'Review detail supports comment sorting query');
  assert(
    detailAfterCommentDelete.data.commentCount === 1,
    'Deleted comment excluded from visible comment count',
  );
  ...
  assert(
    blockedCountCommentRes.status === 201 || blockedCountCommentRes.status === 200,
    `Second comment created for block count check (status ${blockedCountCommentRes.status})`,
  );
  ...
  assert(
    reportReviewRes.status === 201 || reportReviewRes.status === 200,
    `Review report created (status ${reportReviewRes.status})`,
  );
  ...
  assert(
    reportUserRes.status === 201 || reportUserRes.status === 200,
    `User report created (status ${reportUserRes.status})`,
  );

  const blockFriendRes = await request('POST', `/users/${userB.id}/block`, {
    token: userA.tokens.accessToken,
  });
  assert(
    blockFriendRes.status === 201 || blockFriendRes.status === 200,
    `User A blocks User B (status ${blockFriendRes.status})`,
  );

  const detailAfterBlock = await request('GET', `/reviews/${reviewId}`, {
    token: userA.tokens.accessToken,
  });
  assert(
    detailAfterBlock.status === 200,
    'Review author can reload detail after blocking commenter',
  );
  assert(
  ...
  assert(
    detailAfterBlock.data.commentCount === 1,
    'Blocked comment author excluded from review detail count',
  );

  const feedAfterBlock = await request('GET', '/feed', { token: userA.tokens.accessToken });
  const ownFeedReviewAfterBlock = feedAfterBlock.data.items?.find?.(
    (item) => item.reviewId === reviewId,
  );
  assert(
    ownFeedReviewAfterBlock?.commentCount === 1,
    'Blocked comment author excluded from feed count',
  );
  assert(
    !ownFeedReviewAfterBlock?.commentParticipants?.some?.(
      (participant) => participant.id === userB.id,
    ),
    'Blocked comment author excluded from feed participant avatars',
  );

  const blockRes = await request('POST', `/users/${userA.id}/block`, {
    token: userC.tokens.accessToken,
  });
  assert(
    blockRes.status === 201 || blockRes.status === 200,
    `User C blocks User A (status ${blockRes.status})`,
  );

  const searchBlockedRes = await request('GET', `/users/search?q=alice_${suffix}`, {
    token: userC.tokens.accessToken,
  });
  assert(
  ...
  const unblockRes = await request('DELETE', `/users/${userA.id}/block`, {
    token: userC.tokens.accessToken,
  });
  ...
  // ─── Soft Delete Review ───
  console.log('\n--- Review: Soft Delete ---');
  const delRes = await request('DELETE', `/reviews/${reviewId}`, {
    token: userA.tokens.accessToken,
  });
  assert(
    delRes.status === 200 || delRes.status === 204,
    `Review soft-deleted (status ${delRes.status})`,
  );
  ...
  assert(
    updateCarolRes.status === 200,
    `Profile update succeeds (status ${updateCarolRes.status})`,
  );
  assert(updateCarolRes.data.username === nextCarolUsername, 'Profile update changes username');
  assert(
    updateCarolRes.data.email === `carol_${suffix}@test.local`,
    'Email remains active until verification',
  );
  ...
  assert(
    verifyEmailRes.data.user?.email === nextCarolEmail,
    'Verified pending email becomes active email',
  );
  assert(
    verifyEmailRes.data.user?.pendingEmail === null,
    'Pending email clears after verification',
  );
  ...
  assert(
    deleteAccountRes.status === 200,
    `Hard delete account succeeds (status ${deleteAccountRes.status})`,
  );
  ...
  assert(
    loginDeletedRes.status === 401,
    `Deleted account cannot log in (status ${loginDeletedRes.status})`,
  );
  ...
  assert(
    logoutRes.status === 201 || logoutRes.status === 200,
    `Logout succeeds (status ${logoutRes.status})`,
  );
  ...
  assert(
    ...
    `Refresh after logout fails (status ${postLogoutRefresh.status})`,
  );
